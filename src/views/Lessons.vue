<template>
  <div class="flex justify-center mt-12 md:mt-24 mb-12 mx-2">
    ...
    <transition-group id="page" name="fade" class="flex flex-wrap justify-center gap-10 max-w-10xl">
      <LessonCard v-for="lesson in lessonsArray" :key="lesson.id" :imgSrc="lesson.imgSource" :imgAlt="lesson.imgAlt" :heading="lesson.heading" :body="lesson.body" :id="lesson.id" />
    </transition-group>
  </div>
</template>
